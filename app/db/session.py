...
async def init_db() -> None:
    async with engine.begin() as conn:
        ...
        for slug,title,level,body,tags in LESSONS:
            await conn.execute(text("""
                INSERT INTO learning_lessons(slug,title,level,body,tags,active,created_at)
                VALUES (:slug,:title,:level,:body,CAST(:tags AS JSON),true, CURRENT_TIMESTAMP)
                ON CONFLICT (slug) DO UPDATE SET title=EXCLUDED.title, level=EXCLUDED.level, body=EXCLUDED.body, tags=EXCLUDED.tags
            """), {"slug":slug,"title":title,"level":level,"body":body,"tags":json.dumps(tags)})
